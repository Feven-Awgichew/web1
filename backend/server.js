...
const initDb = async () => {
    ...
    // Ensure confirmation_code column exists for tables created by older schema
    try {
        await query(`ALTER TABLE applicants ADD COLUMN IF NOT EXISTS confirmation_code VARCHAR(50)`);
    } catch (e) {
        console.warn('[DB] confirmation_code column check:', e.message);
    }
...
app.post('/api/admin/approve/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await query(
            `UPDATE applicants SET status='approved', confirmation_code=$1, qr_code=$2 WHERE id=$3`,
        ...
        res.json({ message: 'Approved', confirmation_code: confNumber });
    } catch (err) {
...
app.get('/api/badge/:conf', async (req, res) => {
    ...
    const result = await query('SELECT * FROM applicants WHERE confirmation_code=$1', [conf]);
...
app.get('/api/verify/:conf', authenticateToken, async (req, res) => {
    const result = await query('SELECT full_name, role, status, country, confirmation_code FROM applicants WHERE confirmation_code=$1', [req.params.conf]);
